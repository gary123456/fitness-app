...
import { LayoutDashboard, Dumbbell, LogOut, Globe } from "lucide-react";
...
export function Navbar() {
  ...
  // Dictionnaire
  const t = {
    FR: { dash: "Dashboard", prog: "Programme", logout: "Sortir" },
    EN: { dash: "Dashboard", prog: "Workout", logout: "Logout" }
  };
  ...
  const LanguageSelector = () => (
    <DropdownMenu>
      <DropdownMenuTrigger className="flex items-center space-x-2 rounded-md p-2 text-sm font-bold text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900 focus:outline-none">
        ...
        <span className="hidden md:inline">{lang === "FR" ? "🇫🇷 FR" : "🇬🇧 EN"}</span>
      </DropdownMenuTrigger>
  ...
  return (
    <>
        ...
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-8">
          <div className="flex items-center space-x-8">
            <Link href="/dashboard" className="flex items-center">
              <img src="/logo.png" alt="Vivex Logo" className="h-20 w-auto object-contain" />
            </Link>
      ...
      {/* TOP BAR MOBILE (Pour le Logo et le Drapeau) */}
      <div className="md:hidden sticky top-0 z-50 w-full border-b border-zinc-200 bg-white/90 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/90 h-16 flex items-center justify-between px-4">
        <Link href="/dashboard">
          <img src="/logo.png" alt="Vivex Logo" className="h-8 w-auto object-contain" />
        </Link>
        <LanguageSelector />
      </div>
